var padding = 32; // Set this value to define artboard padding, in pixels.
...
function resizeArtboard(artboard, ignoreInvisibles) {
  ...
  //Resize the artboard
  var r = artboard.rect();
  r.size.width = ((maxX - minX)+(padding*2));
  r.size.height = ((maxY - minY)+(padding*2));
  artboard.setRect(r);
  ...
  for (var i = 0; i < layers.count(); i++) {
    ...
    var lr = layer.rect();
    lr.origin.x -= minX-padding;
    lr.origin.y -= minY-padding;
    layer.setRect(lr);
  }
}
...
function throwNoArtboardSelectedError() {
  exitWithError("No artboard selected", "Select one or more artboards");
}
